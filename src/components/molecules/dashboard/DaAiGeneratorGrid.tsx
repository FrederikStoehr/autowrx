...
const WidgetItem: FC<PropsWidgetItem> = ({
  ...
  // Initialize the URL only once
  useEffect(() => {
    if (!widgetConfig || iframeLoaded.current) return
  ...
  // Mark iframe as loaded when it loads
  const handleIframeLoad = () => {
    iframeLoaded.current = true;
    console.log("AI Generator iframe loaded");
  }
...
const DaAiGeneratorGrid: FC<DaDashboardGridProps> = ({ widgetItems, isVisible = true }) => {
  ...
  return (
    <div 
      className={cn(
        'grid h-full w-full grid-cols-5 grid-rows-2',
